def get_free_tables(tables):
    """
    Level 1:
    Returns a list of table IDs that are free in the current timeslot.
    """
    free_tables = []
    for i in range(1, len(tables[0])):  # Start from 1 because 0th index is for table labels
        if tables[1][i] == 'o':  # If the table is free ('o')
...
def find_one_table_for_size(tables, party_size):
    """
    Level 2:
    Returns the first table that can seat the party and is free.
...
def find_all_tables_for_size(tables, party_size):
    """
    Level 3:
    Returns a list of all free tables that can seat at least 'party_size'.
    """
    suitable_tables = []
    for i in range(1, len(tables[0])):  # Loop through tables
        table_label = tables[0][i]
...
def find_tables_including_combos(tables, party_size):
    """
    Level 4:
    Returns a list of single tables or adjacent table combinations that can seat 'party_size'.
    """
    results = []
    for i in range(1, len(tables[0])):  # Loop through tables
        table_label = tables[0][i]
